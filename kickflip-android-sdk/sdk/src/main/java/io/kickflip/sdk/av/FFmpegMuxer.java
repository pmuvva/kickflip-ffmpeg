...
public class FFmpegMuxer extends Muxer implements Runnable {
    ...
    private FFmpegMuxer(String outputFile, FORMAT format, int segDuration) {
        ...
        switch (mFormat) {
            ...
            case HLS:
                ...
                break;
            case FMPEG4:
                opts.outputFormatName = "fmp4";
                break;
            default:
                throw new IllegalArgumentException("Unrecognized format!");
        }

        if ( (mFormat == FORMAT.HLS) || (mFormat == FORMAT.FMPEG4) ) {
            if (segDuration <= 0)
                Log.i(TAG, "Segment duration not specified, assuming default");
            else
                opts.hlsSegmentDurationSec = segDuration;
            Log.i(TAG, "Segment duration set at " + opts.hlsSegmentDurationSec + " seconds");
        }
    ...
    public static FFmpegMuxer create(String outputFile, FORMAT format) {
        return new FFmpegMuxer(outputFile, format, -1);
    }

    public static FFmpegMuxer create(String outputFile, FORMAT format, int segDuration) {
        return new FFmpegMuxer(outputFile, format, segDuration);
    }
    ...
    public void handleAddTrack(MediaFormat trackFormat) {
        ...
        if (!mStarted) {
          ...
          //  mFFmpeg.prepareAVFormatContext(getOutputPath(),mH264Keyframe,mH264MetaSize);
            mStarted = true;
        }
    }
    ...
    private void handleWriteSampleData(MediaCodec encoder, int trackIndex, int bufferIndex, ByteBuffer encodedData, MediaCodec.BufferInfo bufferInfo) {
        ...
        if (((bufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0))
        {
            ...
            if (trackIndex == mVideoTrackIndex) {
                ...
                releaseOutputBufer(encoder, encodedData, bufferIndex, trackIndex);
                mFFmpeg.prepareAdextraData(mH264Keyframe,mH264MetaSize);
       ...
       // bufferInfo.presentationTimeUs = getNextRelativePts(bufferInfo.presentationTimeUs, trackIndex);
        ...
        if (!allTracksFinished()) {
            ...
            if (trackIndex == mVideoTrackIndex && ((bufferInfo.flags & MediaCodec.BUFFER_FLAG_SYNC_FRAME) != 0)) {
               ...
               // Log.i(TAG,"Video pts is "+bufferInfo.presentationTimeUs);

                mFFmpeg.writeAVPacketFromEncodedData(encodedData, (trackIndex == mVideoTrackIndex ? 1 : 0), bufferInfo.offset, bufferInfo.size, bufferInfo.flags, bufferInfo.presentationTimeUs,1);
            ...
            } else
            {
                if(trackIndex == mVideoTrackIndex){
                  //  Log.i(TAG,"Video pts is "+bufferInfo.presentationTimeUs);
                    mFFmpeg.writeAVPacketFromEncodedData(encodedData, (trackIndex == mVideoTrackIndex ? 1 : 0), bufferInfo.offset, bufferInfo.size, bufferInfo.flags, bufferInfo.presentationTimeUs,0);
                }else{
                  //  Log.i(TAG,"Audio pts is "+bufferInfo.presentationTimeUs);
                    if(isVideoSent()){
                        mFFmpeg.writeAVPacketFromEncodedData(encodedData, (trackIndex == mVideoTrackIndex ? 1 : 0), bufferInfo.offset, bufferInfo.size, bufferInfo.flags, bufferInfo.presentationTimeUs,0);
                    }
                }
