...
public class MainActivity extends Activity implements MainFragmentInteractionListener, StreamListFragment.StreamListFragmenListener, AdapterView.OnItemSelectedListener {
    private static final String TAG = "MainActivity";

    private boolean mKickflipReady = false;
    private Spinner resolution;
    private String selectedResolution =null;
   ...
   // private String mRecordingOutputPath = new File(Environment.getExternalStorageDirectory(), "MySampleApp/index.m3u8").getAbsolutePath();
    ...
    private String mRecordingOutputPath = new File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS),"MySampleApp/index.fmp4").getAbsolutePath();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        resolution = (Spinner)findViewById(R.id.spinner1);
        ArrayAdapter<CharSequence> resolutionadapter = ArrayAdapter.createFromResource(
                this, R.array.resolutions_arrays, R.layout.spinner_layout);
        resolutionadapter.setDropDownViewResource(R.layout.spinner_layout);
...
//        Kickflip.setSessionConfig(config);
        ...
        if(selectedResolution.equals("720p")){
            config = Util.create720pSessionConfig(mRecordingOutputPath);
        }
        else if(selectedResolution.equals("1080p")){
            config = Util.create1080pSessionConfig(mRecordingOutputPath);
        }
        else if(selectedResolution.equals("480p")){
            config = Util.create480pSessionConfig(mRecordingOutputPath);
        }
        else if(selectedResolution.equals("240p")){
            config = Util.create240pSessionConfig(mRecordingOutputPath);
        }else{
            Log.e(TAG,"Resolution not matching: "+selectedResolution);
        }
    ...
    public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
        if(i!=0){
            selectedResolution = adapterView.getItemAtPosition(i).toString();
        }else{
            selectedResolution = null;
        }
        if(selectedResolution!=null) {
            Toast.makeText(getApplicationContext(),"Selected Resolution is: "+selectedResolution,Toast.LENGTH_LONG).show();
            // This must happen before any other Kickflip interactions
            Kickflip.setup(this, SECRETS.CLIENT_KEY, SECRETS.CLIENT_SECRET, new KickflipCallback() {
                @Override
                public void onSuccess(Response response) {
